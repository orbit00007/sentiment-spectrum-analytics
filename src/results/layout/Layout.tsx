...
const ChatBubbleButton = ({ onMobileChatOpen }: { onMobileChatOpen: () => void }) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 lg:bottom-8 lg:right-8 no-print z-[9999] group cursor-pointer"
      onClick={handleChatClick}>
      <div className="relative">
        <div className="hidden md:block absolute -inset-2 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-3xl blur-xl opacity-40 group-hover:opacity-60 transition-opacity duration-300 animate-pulse" />
...
export const Layout = ({ children, hideNav }: LayoutProps) => {
  ...
  return (
    <SidebarProvider defaultOpen={false} style={{ "--sidebar-width": "24rem" } as React.CSSProperties}>
      ...
      <SidebarInset className="flex-1 min-w-0 overflow-x-hidden">
        <div className="min-h-screen flex flex-col w-full overflow-x-hidden pt-[56px]" style={{ background: '#F4F6F9' }}>
          <Header />
          {!hideNav && <Navigation />}
        ...
        <ChatBubbleButton onMobileChatOpen={() => setIsMobileChatOpen(true)} />

        {isMobileChatOpen && (
          <div className="fixed inset-0 z-50 md:hidden overflow-hidden" style={{ background: '#FFFFFF' }}>
            <ChatSidebar productId={productId} isMobile={true} onClose={() => setIsMobileChatOpen(false)} />
          </div>
        )}
      </SidebarInset>
    </SidebarProvider>
